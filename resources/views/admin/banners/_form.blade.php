<?php echo Form::open(['url' => isset($model->id) ? route('mainbanners.update', $model->id) : route('mainbanners.store'), 'method' => isset($model->id) ? 'put' : 'post', 'files' => true]) ?>


<div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
    <?php echo Form::label('title', 'Порядковый номер'); ?>
    <?= Form::number('title', isset($model->title) ? $model->title : old('title'), ['class' => 'form-control']) ?>
    @if ($errors->has('title'))
...
<div class="form-group{{ $errors->has('image') ? ' has-error' : '' }}">
    ...
    <div id="imageBox">
        ...
        <a class="btn btn-danger removeImg" data-url='<?= route('mainbanners.edit', $model->id) ?>'>
